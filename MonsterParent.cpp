...
bool MonsterParent::FindAWay() {
	...
	while (!end && d <= intMap.size() * intMap[0].size()) {//ѕока не достигнут пункт назначени€
		//ƒл€ каждой клетки карты,
		for (int i = 0; i < intMap.size(); i++) {
			for (int j = 0; j < intMap[i].size(); j++) {
				// котора€ имеет текущее значение волны,
				if (intMap[i][j] == d) {
					// обходим смежные клетки,
					for (int k = i - 1; k <= i + 1; k++) {
						for (int l = j - 1; l <= j + 1; l++) {
							//не выход€ за границы карты.
							if (k >= 0 && k < intMap.size()) {
								if (l >= 0 && l < intMap[k].size()) {
									//≈сли смежна€ клетка не пункт назначени€
									if (intMap[k][l] != 9999) {
										//и €вл€етс€ проходимой и не помеченной волной
										if (intMap[k][l] == 9998) {
										...
										_currentWay.push_back(IPoint(k, l));//ƒостигнут конец, добавл€ем в путь конечную точку 
										break;//и прекращаем поиск пункта назначени€... 
									}
								}
							}
		...
		++d;// ѕовышаем значение волны
	}


	bool recover = false;
	//¬осстановление пути
	if (_currentWay.size() > 0) {//≈сли найден путь к пункту назначени€
		
		while (d >= 0) {//ѕока значение волны больше нул€
			//ƒл€ всех клеток
			for (int k = 0; k < intMap.size(); k++) {
				...
				//		if (l >= 0 && l < intMap[k].size()) {
					if (intMap[k][l] == d - 1) {//если значение их волны меньше на 1
								//и они наход€тс€ в окрестности текущей клетки
						if (d > 0 && math::abs(_currentWay[_currentWay.size() - 1].x - k) <= 1 && math::abs(_currentWay[_currentWay.size() - 1].y - l <= 1)) {
							_currentWay.push_back(IPoint(k, l));//добавл€ем клетку в путь 
							recover = true;
				...
				if (recover)
					break; //...дл€ текущего значени€ волны
			}
			recover = false;
			--d;//ѕонижаем значение волны
		}
